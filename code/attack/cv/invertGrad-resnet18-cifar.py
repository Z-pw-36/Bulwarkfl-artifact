...
# %%
sensitivity_path = resolve_sensitivity_dir("classification/sensitivity")
...
for cfg_user_idx in [1, 93, 27, 48, 43, 11, 80, 8, 44, 79]:
    ...
    safe_torch_save(results_no_dict, f"./classification/results/{cfg.case.user.user_idx}/{cfg.case.model}_cifar_no_protection.pt")
    ...
    for ratio in [0.001, 0.01, 0.03, 0.05, 0.07, 0.1]:
        results_select_dict = encrypt_selective(sens_list, 
                                                ratio_enc=ratio,
                                                num_repeat=10)
        safe_torch_save(results_select_dict,
                f"./classification/results/{cfg.case.user.user_idx}/{cfg.case.model}_cifar_bulwarkfl_{ratio}.pt")
        ...
        safe_torch_save(results_random_list,
